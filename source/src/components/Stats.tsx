...
const Stats = () => (
  <section className={`${styles.flexCenter} flex-row flex-wrap sm:mb-20 mb-6`} id="features">
    {stats.map((stat) => (
      <div key={stat.id} className={`flex-1 flex justify-start items-center flex-row m-3`}>
        ...
        xs:leading-[26px] leading-[21px] text-white text-gradient ml-3 uppercase">{stat.title}</p>
      </div>
    ))}
  </section>
)
